...
def obfuscate_password(salt, password):
    return base64.b64encode(Scrypt(salt=salt,
                                   length=32,
                                   n=2**19,
                                   r=8,
                                   p=1).derive(password.encode('latin-1'))).decode()
...
class Srp6a (object):
    ...
    def process_challenge(self, bytes_s: bytes, bytes_B: bytes) -> Any:
        ...
        self.p = obfuscate_password(bytes_s, self.p)
